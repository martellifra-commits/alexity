...
st.set_page_config(
    page_title="Itinerario Skopelos & Skiathos",
    page_icon="🏝️",
    layout="wide",
)

st.title("📍 Itinerario di Viaggio: Skopelos & Skiathos")
...
# FILTRO STREAMLIT
# ---------------------------------------------------------
isola_selezionata = st.selectbox(
    "Filtra per Isola:", options=["Tutte", "Skopelos", "Skiathos"]
)

if isola_selezionata != "Tutte":
    df_filtrato = df[df["Isola"] == isola_selezionata]
else:
    df_filtrato = df
